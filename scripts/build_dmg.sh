...
UNINSTALLER_ROOT="$STAGE_ROOT/解除安裝 FamilyRecorder.app"
UNINSTALLER_CONTENTS="$UNINSTALLER_ROOT/Contents"
UNINSTALLER_RESOURCES="$UNINSTALLER_CONTENTS/Resources"
...
install -m 644 \
  ...
  "$PAYLOAD/packaging/FamilyRecorderUninstaller.swift"
install -m 644 \
  "$REPO_ROOT/packaging/UninstallerInfo.plist" \
  "$PAYLOAD/packaging/UninstallerInfo.plist"
...
for script in \
  ...
  uninstall_family_recorder.sh; do
...
echo "Compiling native uninstaller…"
xcrun swiftc -O -parse-as-library -swift-version 5 \
  -target arm64-apple-macos13.0 -framework AppKit \
  "$REPO_ROOT/packaging/FamilyRecorderUninstaller.swift" \
  -o "$UNINSTALLER_CONTENTS/MacOS/FamilyRecorderUninstaller"
install -m 644 \
  "$REPO_ROOT/packaging/UninstallerInfo.plist" \
  "$UNINSTALLER_CONTENTS/Info.plist"
install -m 755 \
  "$REPO_ROOT/scripts/uninstall_family_recorder.sh" \
  "$UNINSTALLER_RESOURCES/uninstall_family_recorder.sh"
plutil -lint "$UNINSTALLER_CONTENTS/Info.plist"
xattr -cr "$UNINSTALLER_ROOT"
codesign --force --deep --options runtime --sign "$SIGN_IDENTITY" "$UNINSTALLER_ROOT"
codesign --verify --deep --strict --verbose=2 "$UNINSTALLER_ROOT"
