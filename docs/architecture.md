...
`analyze_audio` computes RMS and WebRTC VAD before a file is created. In parallel, the XVF3800 auto-selected beam Speech Energy provides independent hardware speech evidence. A software-pass chunk is always retained. A software miss can be rescued only when the configured fraction of auto-selected energy samples is above the firmware speech threshold and local RMS is above `speech_energy_min_rms_dbfs`. A missing control interface therefore degrades to software-only gating rather than aborting audio capture.
